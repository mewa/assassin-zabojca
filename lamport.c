...
int lamport_recv(void* data, unsigned long len, MPI_Datatype dtype, int source,
  ...
  int pos = 0;
  unsigned char recv_buf[MSG_MAX_SIZE];
  unsigned long recv_clk;

  int ret = recv_f(recv_buf, 1, MPI_PACKED, source, tag, comm, status);
  if (ret < 0)
    goto recv_err;
